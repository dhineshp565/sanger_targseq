...
process make_report {
    ...
    Rscript -e "rmarkdown::render(input= 'rmdfile.Rmd', params=list(csv= '${csv}', krona= '${krona}', png='*.png'), output_file = paste0("sanger_targseq_results_report_",format(Sys.time(), "%Y-%m-%d_%H-%M-%S"),".html"))"

	# extract unique case names from the CSV file generated by makecsv.sh
	cut -f 1 -d ',' ${csv} | grep -v "SampleName" | cut -f 1 -d'-' | uniq > caselist.txt
